...
export default function AdminPricingPage() {
  ...
  const load = useCallback(async (manual = false) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || "تعذر تحميل الأسعار.");
      ...
      <AdminShell>
        <main className="page" dir="rtl">
          <div className="center">
            ...
            <strong>جاري تحميل الأسعار</strong>
            <span>يتم جلب بيانات التسعير...</span>
          </div>
  ...
  if (!data) {
    return (
      <AdminShell>
        <main className="page" dir="rtl">
          <div className="center">
            <strong>تعذر تحميل الأسعار</strong>
      ...
      <main className="page" dir="rtl">
        <div className="container">
          <header className="header">
            <div>
              <div className="eyebrow">NINJA ZONE / PRICING</div>
              <h1>إدارة الأسعار</h1>
              <p>تحديد أسعار الخدمات والحد الأدنى لمدة الحجز.</p>
            </div>
          ...
          <section className="pricingGrid">
            {data.length === 0 ? (
              <div className="panel empty">لا توجد أسعار مسجلة.</div>
            ) : (
              data.map((pricing) => (
                <article className={`panel priceCard ${!pricing.active ? "inactive" : ""}`} key={pricing.id}>
                  <div className="priceTop">
                    <div className="priceIcon">{TYPE_ICONS[pricing.resourceType] || "?"}</div>
                    <div className="priceIdentity">
                      <strong>{TYPE_LABELS[pricing.resourceType] || pricing.resourceType}</strong>
                      <span>{pricing.resourceType}</span>
                    </div>
                    <em className={`badge ${pricing.active ? "active" : "inactive"}`}>
                      <i />
                      {pricing.active ? "مفعل" : "معطل"}
                    </em>
                  </div>

                  <div className="priceMain">
                    <strong>{money(pricing.pricePerHour)}</strong>
                    <span>/ ساعة</span>
                  </div>
          ...
          <div className="backdrop">
            <div className="modal" role="dialog" aria-modal="true">
              <div className="modalHead">
                <div>
                  <div className="eyebrow">EDIT PRICING</div>
                  <h2>تعديل سعر {TYPE_LABELS[edit.resourceType] || edit.resourceType}</h2>
                </div>
              ...
              <div className="form">
                ...
                <label>
                  الحد الأدنى للمدة (دقيقة)
                  <select
                    value={edit.minMinutes}
                    disabled={saving}
                    onChange={(e) => setEdit({ ...edit, minMinutes: Number(e.target.value) })}
                  >
                ...
                <div className="switchRow">
                  <div>
                    <strong>السعر مفعل</strong>
                    <small>المعطل لا يظهر للعملاء.</small>
                  </div>
                  <button
