  ...
  const load = useCallback(
    async (manual = false) => {
      ...
      try {
        ...
        if (!response.ok) throw new Error(result.error || "تعذر تحميل المستخدمين.");
        setData(result);
        setError("");
      } catch (err) {
  ...
  async function createUser() {
    ...
    try {
      const response = await fetch("/api/admin/users", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: createOpen.name,
          phone: createOpen.phone,
          password: createOpen.password,
          role: createOpen.role,
        }),
      });
      const result = await response.json().catch(() => ({}));
      ...
      <AdminShell>
        <main className="adminPage" dir="rtl">
          <div className="center">
            ...
            <strong>جاري تحميل العملاء</strong>
            <span>يتم جلب بيانات المستخدمين...</span>
          </div>
      ...
      <main className="adminPage" dir="rtl">
        <div className="container">
          <header className="header">
            <div>
              <div className="eyebrow">NINJA ZONE / CUSTOMERS</div>
            ...
            <div className="headerActions">
              ...
              <button className="primary" onClick={() => setCreateOpen({ name: "", phone: "", password: "", role: "CUSTOMER" })}>
                + مستخدم جديد
              </button>
            </div>
          </header>
          ...
          <section className="summary">
            <Stat title="إجمالي المستخدمين" value={data.total} tone="purple" />
            <Stat title="عملاء" value={data.roleCounts.CUSTOMER ?? 0} tone="green" />
            <Stat title="كاشير" value={data.roleCounts.CASHIER ?? 0} tone="gold" />
            <Stat title="أدمن / مدير" value={(data.roleCounts.ADMIN ?? 0) + (data.roleCounts.MANAGER ?? 0)} tone="red" />
          </section>
          ...
          <div className="backdrop">
            <div className="modal" role="dialog" aria-modal="true">
              ...
              <div className="modalActions">
                ...
                <button className="primary" disabled={saving} onClick={createUser}>
...
@media(max-width:650px){.adminPage{padding:18px 10px 40px}.header{flex-direction:column;align-items:stretch}.headerActions{display:grid;grid-template-columns:1fr 1fr}.headerActions>*{width:100%}.summary{grid-template-columns:1fr}.filters{grid-template-columns:1fr}.reset{grid-column:auto}.modal{padding:15px}}
`;