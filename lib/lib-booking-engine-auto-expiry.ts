...
function assertDate(value: string, label: string): Date {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new BookingValidationError(
      `القيمة الخاصة بـ ${label} ليست تاريخًا أو وقتًا صالحًا.`,
    );
  }
...
export function normalizeBookingItems(
  ...
  return items.map((item) => {
    ...
    if (
      ...
      throw new BookingValidationError(
        "الكمية يجب أن تكون رقمًا صحيحًا من 1 إلى 50.",
      );
    }
    ...
    if (startAt > latestAllowed) {
      throw new BookingValidationError(
        "يمكن الحجز لمدة تصل إلى 7 أيام من الآن فقط.",
      );
    }
...
async function finalizeExpiredActiveBookings(now: Date = new Date()) {
...
/**
 * Ninja Zone canonical resource codes.
 *
 * This prevents old/accidental resource rows such as:
 * BIL-01 / BIL-02
 *
 * from appearing beside:
...
function filterCanonicalResources<
  T extends {
    id: string;
    code: string;
    status: string;
...
>(resources: T[]): T[] {
  return resources.filter((resource) =>
    isCanonicalResourceCode(
      resource.type ?? ResourceType.BILLIARD,
      resource.code,
    ),
  );
}
...
async function allocateResources(
  ...
  if (
    ...
    throw new BookingValidationError(
      "عدد الأجهزة المحددة يجب أن يطابق عدد الأجهزة المطلوبة.",
    );
  }
  ...
  if (
    resources.length < item.quantity
  ) {
    if (requestedIds.length > 0) {
      throw new BookingValidationError(
        "جهاز واحد أو أكثر من الأجهزة المحددة لم يعد متاحًا.",
      );
    }

    throw new BookingValidationError(
      `عدد ${item.resourceType} المتاح غير كافٍ لإتمام الحجز.`,
    );
  }
  ...
  if (
    ...
    throw new BookingValidationError(
      `المتاح حاليًا من ${item.resourceType} هو ${available.length} فقط للوقت المحدد.`,
    );
  }
    ...
    try {
      return await prisma.$transaction(
        async (tx) => {
          ...
          if (!user) {
            throw new BookingValidationError(
              "لم يتم العثور على حساب العميل.",
            );
          }

          if (!user.isActive) {
            throw new BookingValidationError(
              "حساب العميل غير فعال.",
            );
          }
          ...
          for (const item of normalized) {
            ...
            if (
              ...
              throw new BookingValidationError(
                `لا يوجد سعر فعال ومحدد لـ ${item.resourceType}.`,
              );
            }
...
export async function getAvailability(
  ...
  /**
   * Only canonical resources are exposed.
   *
   * For BILLIARD this means exactly:
   * BILLIARD-01
   * BILLIARD-02
   */
  const resources =
...
export function makeIraqDate(
  ...
  if (
    ...
    throw new BookingValidationError(
      "استخدم التاريخ بصيغة YYYY-MM-DD والوقت بصيغة HH:mm.",
    );
  }
...
export function generateBusinessDaySlots(
  ...
  if (
    ...
    throw new BookingValidationError(
      "تاريخ العمل يجب أن يكون بصيغة YYYY-MM-DD.",
    );
  }
